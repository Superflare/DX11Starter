...
void Game::Init()
{
	...
	std::shared_ptr<Material> mDarkTint = std::make_shared<Material>(vertexShader, pixelShader, XMFLOAT4(0.4f, 0.4f, 0.4f, 1));
	...
	// Not every mesh used in the game entities is centered at the origin so transformations are relative
	{
		...
		// Quad Double Sided
		entities.push_back(std::make_shared<GameEntity>(meshes[4], mNoTint));
		// Sphere
		entities.push_back(std::make_shared<GameEntity>(meshes[5], mDarkTint));
		// Torus
		entities.push_back(std::make_shared<GameEntity>(meshes[6], mRedTint));
