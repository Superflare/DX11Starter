	...
	)
{
	...
	if (ImGui::BeginTabBar("Scene Components"))
	{
		...
		if (ImGui::BeginTabItem("Entities"))
		{
			...
			for (int i = 0; i < entities.size(); i++)
			{
				...
				if (ImGui::TreeNode("Entity Node", "Entity %d", i))
				{
					...
					{
						XMFLOAT3 change;
						XMStoreFloat3(&change, XMLoadFloat3(&rot) - XMLoadFloat3(&oldRot));
						transform->RotatePitchYawRollWorld(Deg2RadFromVector(change));
					}
		...
		if (ImGui::BeginTabItem("Lights"))
		{
			...
			{
				ImGui::PushID(i);
				if (ImGui::TreeNode("Light Node", "(%s) Light %d", LightTypeToString(lights[i].type), i))
				{
